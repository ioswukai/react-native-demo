...
export default class ES6NumberExtension extends Component<Props> {
    ...
    componentDidMount() {
        ...
        // Number.isInteger()
        this.numberIsInteger();

        // Number.EPSILON
        this.numberEPSILON();

        // 安全整数和 Number.isSafeInteger()
        this.numberIsSafeInteger();
    ...
    number2and8JinZhi(){
        ...
        // true
        console.log("0b111110111 === 503 is ",0b111110111 === 503);
        console.log("0o767 === 503 is ",0o767 === 503);
    ...
    /*****************Number.isFinite(), Number.isNaN()  *****************/
    numberIsFinite(){
        /*ES6 在Number对象上，新提供了Number.isFinite()
        和Number.isNaN()两个方法。*/
    ...
    numberIsSafeInteger(){
        ...
        /*实际使用这个函数时，需要注意。验证运算结果是否落在安全整数的范围内，
        ...
        /*上面代码中，9007199254740993不是一个安全整数，但是Number.isSafeInteger会返回结果，
        显示计算结果是安全的。这是因为，这个数超出了精度范围，导致在计算机内部，
    ...
    /*****************Math 对象的扩展  *****************/
    numberMath(){
        /*ES6 在 Math 对象上新增了 17 个与数学相关的方法。所有这些方法都是静态方法，
        只能在 Math 对象上调用。*/
        ...
        /*对于没有部署这个方法的环境，可以用下面的代码模拟。*/
         ...
         /*Math.clz32()
         JavaScript 的整数使用 32 位二进制形式表示，Math.clz32方法返回一个数的 32
         位无符号整数形式有多少个前导 0。*/
         ...
         只占 1 位，所以 32 位之中有 31 个前导 0；1000 的二进制形式是0b1111101000，一共有 10 位，
         所以 32 位之中有 22 个前导 0。
         ...
         即该方法等同于(a * b)|0的效果（超过 32 位的部分溢出）。之所以需要部署这个方法，
         是因为 JavaScript 有精度限制，超过 2 的 53 次方的值无法精确表示。这就是说，对于那些很大的数的乘法，
         ...
         /*对于 NaN 和 Infinity，此方法返回原值。对于其它类型的非数值，Math.fround 方法会先将其转为数值，
         再返回单精度浮点数。*/
