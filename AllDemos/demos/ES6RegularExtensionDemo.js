...
export default class ES6RegularExtension extends Component<Props> {
    ...
    componentDidMount() {
        ...
        // 字符串的正则方法
        this.regularStrMethod();

        // u 修饰符
        this.regularUXiuShiFu();

        // y 修饰符
        this.regularYXiuShiFu();
        ...
        // 后行断言
        this.regularHouHangDuanYan();

        // Unicode 属性类
        this.regularUnicode();

        // 具名组匹配
        this.regularJuMingZuPiPei();

        // String.prototype.matchAll
        this.regularMatchAll();
    ...
    regularRegExp(){
        ...
        console.log("new RegExp('xyz', 'i')  =",regex1);

        /*第二种情况是，参数是一个正则表示式，这时会返回一个原有正则表达式的拷贝。*/
        let regex2 = new RegExp(/xyz/i);
        // new RegExp(/xyz/i)  = /xyz/i
        console.log("new RegExp(/xyz/i)  =",regex2);

        // 上面两种情况regex1 regex2 都等价于
        let regex = /xyz/i;


        /*但是，ES5 不允许此时使用第二个参数添加修饰符，否则会报错。*/
        // Uncaught TypeError: Cannot supply flags when constructing one RegExp from another
        // let regex3 = new RegExp(/xyz/, 'i');
        ...
        let regex4 = new RegExp(/abc/ig, 'i').flags
        console.log("new RegExp(/abc/ig, 'i').flags  =",regex4);
    ...
    regularUXiuShiFu(){
        ...
        //  /^\uD83D/u.test('\uD83D\uDC2A') // false
        // /^\uD83D/.test('\uD83D\uDC2A') // true
        /*上面代码中，\uD83D\uDC2A是一个四个字节的 UTF-16 编码，代表一个字符。但是，
    ...
    regularYXiuShiFu(){
        /*除了u修饰符，ES6 还为正则表达式添加了y修饰符，叫做“粘连”（sticky）修饰符。
         ...
         /b/y.exec('aba')
         // null
         上面代码由于不能保证头部匹配，所以返回null。y修饰符的设计本意，
         就是让头部匹配的标志^在全局匹配中都有效。
    ...
    regularDoAll(){
        ...
        一个是四个字节的 UTF-16 字符，这个可以用u修饰符解决；另一个是行终止符（line terminator character）。
         ...
         /foo.bar/s.test('foo\nbar') // true
         这被称为dotAll模式，即点（dot）代表一切字符。所以，正则表达式还引入了一个dotAll属性，
    ...
    regularHouHangDuanYan(){
        ...
        ES2018 引入后行断言，V8 引擎 4.9 版（Chrome 62）已经支持。
         ...
         “后行断言”正好与“先行断言”相反，x只有在y后面才匹配，必须写成/(?<=y)x/。比如，只匹配美元符号之后的数字，
         要写成/(?<=\$)\d+/。”后行否定断言“则与”先行否定断言“相反，x只有不在y后面才匹配，必须写成/(?<!y)x/。
         ...
         “后行断言”的实现，需要先匹配/(?<=y)x/的x，然后再回到左边，匹配y的部分。这种“先右后左”的执行顺序，
         与所有其他正则操作相反，导致了一些不符合预期的行为。
         ...
         所以结果是105和3。而“后行断言”时，由于执行顺序是从右到左，第二个括号是贪婪模式，第一个括号只能捕获一个字符，
         所以结果是1和053。
    ...
    regularUnicode(){
        /*ES2018 引入了一种新的类的写法\p{...}和\P{...}，允许正则表达式匹配符合 Unicode 某种属性的所有字符。
         ...
         注意，这两种类只对 Unicode 有效，所以使用的时候一定要加上u修饰符。如果不加u修饰符，
         正则表达式使用\p和\P会报错，ECMAScript 预留了这两个类。
    ...
    regularJuMingZuPiPei(){
        /*简介
         ...
         上面代码中，具名组as没有找到匹配，那么matchObj.groups.as属性值就是undefined，
    ...
    regularMatchAll(){
        /*如果一个正则表达式在字符串里面有多个匹配，现在一般使用g修饰符或y修饰符，在循环里面逐一取出。
         ...
         目前有一个提案，增加了String.prototype.matchAll方法，可以一次性取出所有匹配。不过，
